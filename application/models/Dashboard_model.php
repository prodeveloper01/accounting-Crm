...
class Dashboard_model extends CI_Model
{
    ...
    public function incomeChart($today, $month, $year)
    {
        ...
        $query = $this->db->query("SELECT SUM(credit) AS total,date FROM transactions WHERE ((DATE(date) BETWEEN DATE('$year-$month-01') AND CURDATE()) AND type='Income') GROUP BY date");
        return $query->result_array();
    }
    ...
    public function expenseChart($today, $month, $year)
    {
        ...
        $query = $this->db->query("SELECT SUM(debit) AS total,date FROM transactions WHERE ((DATE(date) BETWEEN DATE('$year-$month-01') AND CURDATE()) AND type='Expense') GROUP BY date");
        return $query->result_array();
    }
    ...
    public function countmonthlyChart()
    {
        ...
        $query = $this->db->query("SELECT COUNT(id) AS ttlid,SUM(total) AS total,DATE(invoicedate) as date FROM invoices WHERE (DATE(invoicedate) BETWEEN CURDATE() - INTERVAL 30 DAY AND CURDATE()) GROUP BY date");
        return $query->result_array();
    }

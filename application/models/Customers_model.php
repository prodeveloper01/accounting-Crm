...
class Customers_model extends CI_Model
{
    ...
    var $rec_inv_column_order = array(null, 'tid', 'name', 'invoicedate', 'total', 'status', null);
    var $rec_inv_column_search = array('tid', 'name', 'invoicedate', 'total');
    var $rec_inv_order = array('rec_invoices.tid' => 'desc');
    ...
    public function inv_count_all($id)
    {
       ...
       private function _rec_inv_datatables_query($id)
    {

        $this->db->from('rec_invoices');
        $this->db->where('rec_invoices.csd', $id);
        ...
        foreach ($this->inv_column_search as $item) // loop column
        {
            if ($_POST['search']['value']) // if datatable send POST for search
            {
                ...
                if (count($this->rec_inv_column_search) - 1 == $i) //last loop
                    $this->db->group_end(); //close bracket
            }
            $i++;
        }
        ...
        if (isset($_POST['order'])) // here order processing
        {
            $this->db->order_by($this->rec_inv_column_order[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
        } else if (isset($this->rec_inv_order)) {
            $order = $this->rec_inv_order;
            $this->db->order_by(key($order), $order[key($order)]);
        }
    }

    function rec_inv_datatables($id)
    {
        $this->_rec_inv_datatables_query($id);
        if ($_POST['length'] != -1)
            $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
        return $query->result();
    ...
    function rec_inv_count_filtered($id)
    {
        $this->_rec_inv_datatables_query($id);
        $query = $this->db->get();
        return $query->num_rows();
    ...
    public function rec_inv_count_all($id)
    {
        $this->db->from('rec_invoices');
        $this->db->where('csd', $id);
        return $this->db->count_all_results();

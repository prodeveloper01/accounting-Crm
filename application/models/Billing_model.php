...
class Billing_model extends CI_Model
{
    ...
    public function paynow($tid, $amount, $note, $pmethod,$inv='inv')
    {
        ...
        if($inv=='inv') {
            ...
            if ($totalrm > $amount) {
             ...
             $this->db->select('rec_invoices.*,customers.name,customers.id AS cid');
            $this->db->from('rec_invoices');
            $this->db->where('rec_invoices.tid', $tid);

                                    ...
                                    <div class="col-md-8">
                                        <div class="form-group mt-2"><?php echo $this->lang->line('Payment') ?>:
                                            <?php if ($online_pay['enable'] == 1) {
                                                echo '<a class="btn btn-success btn-min-width mr-1" href="' . base_url('billing/card?id=' . $invoice['tid'] . '&itype=rinv&token=' . $token) . '" data-toggle="modal" data-target="#paymentCard" ><i class="icon-cc"></i> Credit Card</a> ';
                                            }
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    foreach ($gateway as $row) {
                        ...
                        if ($row['surcharge'] > 0) {
                            $surcharge_t = true;
                            $fee = '( ' . amountExchange($invoice['total'], $invoice['multi']) . '+' . amountFormat_s($row['surcharge']) . ' %)';
                        } else {
                            $fee = '';
                        }

                        echo '<a href="' . base_url('billing/card?id=' . $invoice['tid'] . '&itype=rinv&token=' . $token) . '&gid=' . $cid . '" class="btn mb-1 btn-block blue rounded border border-info text-bold-700 border-lighten-5 "><span class=" display-block"><span class="grey">Pay With </span><span class="blue font-medium-2">' . $title . ' ' . $fee . '</span></span>
